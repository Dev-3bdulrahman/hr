<div>
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-2xl font-black text-gray-900 dark:text-white">{{ __('hr::hr.attendance_logs') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('hr::hr.manage_attendance') }}</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                <tr>
                    ...
                    <th class="text-start px-4 py-4 font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest text-[10px]">{{ __('hr::hr.date') }}</th>
                    <th class="text-start px-4 py-4 font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest text-[10px]">{{ __('hr::hr.check_in') }}</th>
                    <th class="text-start px-4 py-4 font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest text-[10px]">{{ __('hr::hr.check_out') }}</th>
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    <td class="px-6 py-4 font-bold text-gray-900 dark:text-white">{{ $att->employee->fullName }}</td>
                    <td class="px-4 py-4 text-gray-500">{{ $att->date->toDateString() }}</td>
                    <td class="px-4 py-4 text-gray-500 font-mono">{{ $att->check_in }}</td>
                    <td class="px-4 py-4 text-gray-500 font-mono">{{ $att->check_out ?? '—' }}</td>
                    <td class="px-4 py-4">
                        <span class="px-2.5 py-1 text-xs font-bold rounded-full {{ $att->status === 'present' ? 'bg-green-50 text-green-700 dark:bg-green-900/20' : ($att->status === 'late' ? 'bg-yellow-50 text-yellow-700 dark:bg-yellow-900/20' : 'bg-red-50 text-red-700 dark:bg-red-900/20') }}">
                            {{ __('hr::hr.' . $att->status) }}
                        </span>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="5" class="px-6 py-16 text-center text-gray-400">
                        <i data-lucide="clock" class="w-12 h-12 mx-auto mb-4 text-gray-200 dark:text-gray-700"></i>
                        <p>{{ __('hr::hr.no_attendance_yet') }}</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm"
        ...
        <div class="bg-white dark:bg-gray-900 w-full max-w-lg rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800">
            <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <h3 class="text-lg font-black text-gray-900 dark:text-white">{{ __('hr::hr.log_attendance') }}</h3>
            ...
            <div class="px-8 py-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1.5">{{ __('hr::hr.date') }} *</label>
                    <input wire:model="date" type="date" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                    @error('date') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 mb-1.5">{{ __('hr::hr.check_in') }} *</label>
                        <input wire:model="check_in" type="time" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                        @error('check_in') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 mb-1.5">{{ __('hr::hr.check_out') }}</label>
                        <input wire:model="check_out" type="time" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
