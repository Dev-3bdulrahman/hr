<div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm mb-6">
        <div class="space-y-1">
            <h1 class="text-2xl font-black text-gray-900 dark:text-white">{{ __('hr::hr.designations') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('hr::hr.manage_designations') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4 mb-6">
        <div class="relative flex items-center">
            <input wire:model.live.debounce.300ms="search" type="text"
                placeholder="{{ __('hr::hr.search_designations') }}"
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
                <tr>
                    <th class="text-start px-6 py-4 font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest text-[10px]">{{ __('hr::hr.designation_name') }}</th>
                    <th class="text-start px-4 py-4 font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest text-[10px]">{{ __('hr::hr.designation_level') }}</th>
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    <td class="px-6 py-4 font-bold text-gray-900 dark:text-white">{{ $desg->name }}</td>
                    <td class="px-4 py-4 text-gray-500">{{ $desg->level ?? '—' }}</td>
                    <td class="px-4 py-4">
                        <span class="bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 text-xs font-bold px-2.5 py-1 rounded-full">
                            {{ $desg->employees_count }}
                        </span>
                    </td>
                    <td class="px-4 py-4">
                        <span class="px-2.5 py-1 text-xs font-bold rounded-full {{ $desg->status === 'active' ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-800' }}">
                            {{ __('hr::hr.' . $desg->status) }}
                        </span>
                    </td>
                    <td class="px-4 py-4">
                        <div class="flex items-center gap-1">
                            <button wire:click="openModal({{ $desg->id }})" title="{{ __('hr::hr.edit_designation') }}"
                            ...
                            <button wire:click="$dispatch('swal:confirm', {
                                    ...
                                    text: '{{ __('hr::hr.delete_designation_confirm') }}',
                                    onConfirm: 'delete',
                                    params: { id: {{ $desg->id }} }
                                })"
                ...
                <tr>
                    <td colspan="5" class="px-6 py-16 text-center text-gray-400">
                        <i data-lucide="briefcase" class="w-12 h-12 mx-auto mb-4 text-gray-200 dark:text-gray-700"></i>
                        <p>{{ __('hr::hr.no_designations_yet') }}</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm"
        ...
        <div class="bg-white dark:bg-gray-900 w-full max-w-md rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800">
            <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <h3 class="text-lg font-black text-gray-900 dark:text-white">
                    {{ $designation_id ? __('hr::hr.edit_designation') : __('hr::hr.add_designation') }}
                </h3>
            ...
            <div class="px-8 py-6 space-y-4">
                <div>
                    <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-1.5">{{ __('hr::hr.designation_name') }} *</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-1.5">{{ __('hr::hr.designation_level') }}</label>
                    <input wire:model="level" type="text" placeholder="{{ __('hr::hr.designation_level') }}"
